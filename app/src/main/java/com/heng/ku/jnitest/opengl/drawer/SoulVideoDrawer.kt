...
//--------------------------------------
    ...
    override fun setVideoWh(videoW: Int, videoH: Int) {
        mVideoWidth = videoW
        mVideoHeight = videoH
    }

    override fun setWordWh(worldW: Int, worldH: Int) {
        mWorldWidth = worldW
        mWorldHeight = worldH
    }
    ...
    private fun updateFBO() {
        if (mSoulTextureId == -1) {
            // 创建FBO纹理
            mSoulTextureId = OpenGLTools.createFBOTexure(mVideoWidth, mVideoHeight)[0]
        }
