...
void NV21TOYUV420SP(const unsigned char *src,const unsigned char *dst,int ySize)
{
	...
	unsigned char *nvcur = (unsigned char *) (src + ySize);
	unsigned char *yuvcur = (unsigned char *) (dst + ySize + 1);
	int i=0;
	while(i<uSize)
	{
		(*yuvcur)=(*nvcur);
...
void NV21TOYUV420P(const unsigned char *src,const unsigned char *dst,int ySize)
{
	memcpy((void *) dst, src, ySize);
	int uSize = ySize>>2;
	unsigned char *srcucur = (unsigned char *) (src + ySize + 1);
	unsigned char *srcvcur = (unsigned char *) (src + ySize);
	unsigned char *dstucur = (unsigned char *) (dst + ySize);
	unsigned char *dstvcur = (unsigned char *) (dst + ySize + uSize);
	int i=0;
	while(i<uSize)
	{
		(*dstucur)=(*srcucur);
...
void YUV420SPTOYUV420P(const unsigned char *src,const unsigned char *dst,int ySize)
{
	memcpy((void *) dst, src, ySize);
	int uSize = ySize>>2;
	unsigned char *srcucur = (unsigned char *) (src + ySize);
	unsigned char *srcvcur = (unsigned char *) (src + ySize + 1);
	unsigned char *dstucur = (unsigned char *) (dst + ySize);
	unsigned char *dstvcur = (unsigned char *) (dst + ySize + uSize);
	int i=0;
	while(i<uSize)
...
void NV21TOARGB(const unsigned char *src,const unsigned int *dst,int width,int height)
{
	...
	unsigned int *target= (unsigned int *) dst;
...
void NV21TOYUV(const unsigned char *src,const unsigned char *dstY,const unsigned char *dstU,const unsigned char *dstV,int width,int height)
{
	...
	unsigned char *srcucur = (unsigned char *) (src + ySize + 1);
	unsigned char *srcvcur = (unsigned char *) (src + ySize);
	unsigned char *dstucur = (unsigned char *) dstU;
	unsigned char *dstvcur = (unsigned char *) dstV;
	int i=0;
	while(i<uSize)
	{
		(*dstucur)=(*srcucur);
